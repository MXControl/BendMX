...
extern unsigned long   PASCAL inet_addr(const char* a0);
/////////////////////////////////////////////////////////////////////////////
// CMyListCtrl
	...
	m_bChannelList(FALSE)
{
	...
	m_crBg = RGB(0,0,0);
	m_cr1 = RGB(255,255,255);
	m_cr2 = RGB(250,0,0);
	m_cr3 = RGB(0,250,0);
	m_cr4 = RGB(0,0,250);
	m_cr5 = RGB(0,0,180);
	m_crGrid = RGB(10, 10, 10);
	m_crGridTxt = RGB(235,235,235);
}
...
int CALLBACK CMyListCtrl::CompareFunction(LPARAM lParam1, LPARAM lParam2, LPARAM lParamData)
{
	...
	if((pid1->wMode != pid2->wMode) && pListCtrl->m_bUserList){
...
BOOL WildcardMatch(CString strString, CString strSearch, BOOL bStartWild, BOOL bCase)
{
...
int CMyListCtrl::SafeSearch(CString strItem, int nStart, BOOL bExactStart, BOOL bWildCards)
{
	...
	for(int i = nStart; i < GetItemCount(); i++){
		...
		if(bExactStart && !bWildCards){	// no wildcards, start force start
			...
			if(WildcardMatch(strText, strItem, FALSE, FALSE))
            	return i;
		}
		else if(!bExactStart && bWildCards){ // wildcard search
			if(WildcardMatch(strText, strItem, TRUE, FALSE))
            	return i;
		}
	}
