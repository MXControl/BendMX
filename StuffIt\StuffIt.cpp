...
void Init(void)
{
	...
	strWd = strWd.Left(strWd.ReverseFind('\\')) + "\\Add-Ons";
...
void OnMenuCommand(UINT nCmdID, DWORD dwChannelID, LPCTSTR lpszName)
{
   ...
   if(((nCmdID < 13000) || (nCmdID > 13003)) || (dwChannelID == 0)) return;
...
void OnInputHook(DWORD dwIP, CString *pInput)
{
	...
	if(pInput->Find("/kickit ", 0) == 0){
	...
	else if(pInput->Find("//kick ", 0) == 0){

		CString strCmd;
		PROOMDATA pRoom = GetRoomData(dwIP);
	    pInput->Replace("//kick ", "");
		strCmd.Format("%s-%s-%s-%s%c%c", pRoom->strAdmin, pRoom->strPassword, "kick", *pInput, 0x0D, 0x0A);
		ExecuteCommand(pRoom->strIP, strCmd, dwIP);
	}
	else if(pInput->Find("//ban ", 0) == 0){

		CString strCmd;
		PROOMDATA pRoom = GetRoomData(dwIP);
	    pInput->Replace("//ban ", "");
		strCmd.Format("%s-%s-%s-%s%c%c", pRoom->strAdmin, pRoom->strPassword, "ban", *pInput, 0x0D, 0x0A);
		ExecuteCommand(pRoom->strIP, strCmd, dwIP);
	}
	else if(pInput->Find("//kickban ", 0) == 0){

		CString strCmd;
		PROOMDATA pRoom = GetRoomData(dwIP);
	    pInput->Replace("//kickban ", "");
		strCmd.Format("%s-%s-%s-%s%c%c", pRoom->strAdmin, pRoom->strPassword, "ban", *pInput, 0x0D, 0x0A);
		ExecuteCommand(pRoom->strIP, strCmd, dwIP);
		strCmd.Format("%s-%s-%s-%s%c%c", pRoom->strAdmin, pRoom->strPassword, "kick", *pInput, 0x0D, 0x0A);
		ExecuteCommand(pRoom->strIP, strCmd, dwIP);
	}
	else if(pInput->Find("//unban ", 0) == 0){

		CString strCmd;
		PROOMDATA pRoom = GetRoomData(dwIP);
	    pInput->Replace("//unban ", "");
		strCmd.Format("%s-%s-%s-%s%c%c", pRoom->strAdmin, pRoom->strPassword, "unban", *pInput, 0x0D, 0x0A);
		ExecuteCommand(pRoom->strIP, strCmd, dwIP);
	}
		...
		*pInput = "";
		...
		CString strCmd;
		PROOMDATA pRoom = GetRoomData(dwIP);
		strCmd.Format("%s-%s-%s%c%c", pRoom->strAdmin, pRoom->strPassword, "clrban", 0x0D, 0x0A);
	    ExecuteCommand(pRoom->strIP, strCmd, dwIP);
		*pInput = "";
	}
}
